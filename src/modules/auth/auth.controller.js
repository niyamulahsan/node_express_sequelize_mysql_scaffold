...
auth.login = async (req, res, next) => {
  try {
    ...
    const userdata = await User.findOne({ include: ["role"], where: { email: data.email } });
    ...
    const token = jwt.sign({ id: userdata.id, email: userdata.email, role: userdata.role.name, status: userdata.status }, process.env.TOKEN_SECRET, { expiresIn: process.env.TOKEN_EXPIRE });
